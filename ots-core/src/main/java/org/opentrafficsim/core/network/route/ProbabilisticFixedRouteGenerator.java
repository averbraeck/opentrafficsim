...
public class ProbabilisticFixedRouteGenerator implements RouteGenerator
{
     ...
     * @throws Exception when the probabilities or frequencies are invalid (negative, or all zero)
     */
    public ProbabilisticFixedRouteGenerator(SortedMap<RouteGenerator, Double> routeProbabilities) throws Exception
    {
            ...
            throws Exception
    {
        ...
        for (RouteGenerator rg : routeProbabilities.keySet())
        {
            ...
            if (frequency < 0)
            {
                throw new Exception("Negative probability or frequency is not allowed (got " + frequency + ")");
            }
            sum += frequency;
            this.routes.add(rg);
        }
        if (0 == sum)
        {
            throw new Exception("Sum of probabilities or freqencies must be > 0");
        }
        ...
        this.random = new DistUniform(new MersenneTwister(), 0, 1);
    }
