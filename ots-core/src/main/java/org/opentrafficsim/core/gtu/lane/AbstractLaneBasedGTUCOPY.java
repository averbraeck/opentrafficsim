 ...
 */
public abstract class AbstractLaneBasedGTUCOPY<ID> extends AbstractGTU<ID> implements LaneBasedGTU<ID>
{
    /** */
     ...
     */
    public AbstractLaneBasedGTUCOPY(final ID id, final GTUType<?> gtuType, final GTUFollowingModel gtuFollowingModel,
    ...
    {
        if (this.getId().toString().contains("118"))
        {
    ...
    protected final void move() throws RemoteException, NamingException, NetworkException, SimRuntimeException
    {
        ...
        if (getId().toString().equals("79") && getSimulator().getSimulatorTime().get().getSI() > 7.9)
        {
        ...
        else
        {
            ...
            HeadwayGTU leader = headway(maximumForwardHeadway);
            AccelerationStep as =
                null != leader ? getGTUFollowingModel().computeAcceleration(this, leader.getOtherGTU().getLateralVelocity(),
