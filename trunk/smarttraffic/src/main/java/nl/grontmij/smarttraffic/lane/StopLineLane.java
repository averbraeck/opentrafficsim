 ...
 * <p>
 * 
 * @version $Revision$, $LastChangedDate$, by $Author$, initial version Jan 1,
 *          2015 <br>
...
public class StopLineLane extends AbstractSensor {
	 ...
	 *            longitudinal position from the end TODO change for position,
	 *            not from the end.
	 */
	...
	public StopLineLane(final Lane lane,
		...
		super(lane, longitudinalPositionFromEnd, RelativePosition.FRONT,
				"STOPLINE@" + lane.toString(), simulator);
	}

	public StopLineLane(String name, final Lane lane,
			final DoubleScalar.Rel<LengthUnit> longitudinalPosition,
			OTSDEVSSimulatorInterface simulator) {
		super(lane, longitudinalPosition, RelativePosition.FRONT, name,
				simulator);
		try {
			this.trafficLight = new TrafficLight(name, this.getLane(),
					this.getLongitudinalPosition(), simulator);
	 ...
	 * 
	 * @param simulator
	 *            the simulator for the stop GTU
	 * @throws RemoteException
	 *             on communications failure
	 */
	protected final void createStopGTU(final OTSDEVSSimulatorInterface simulator)
			throws RemoteException {
		/** Type of all GTUs. */
		try {
			Map<Lane, DoubleScalar.Rel<LengthUnit>> initialPositions = new LinkedHashMap<Lane, DoubleScalar.Rel<LengthUnit>>();
			initialPositions
					.put(this.getLane(), this.getLongitudinalPosition());
			this.stopGTU = new LaneBasedIndividualCar("999999",
					GTUType.makeGTUType("CAR"), new IDMPlus(), new Egoistic(),
					initialPositions, new DoubleScalar.Abs<SpeedUnit>(0,
					...
					new CompleteLaneBasedRouteNavigator(new CompleteRoute("")),
					simulator, DefaultCarAnimation.class, new IDGTUColorer());
