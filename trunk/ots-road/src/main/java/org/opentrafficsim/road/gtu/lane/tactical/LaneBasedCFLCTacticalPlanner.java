...
public class LaneBasedCFLCTacticalPlanner extends AbstractLaneBasedTacticalPlanner
{
        ...
        try
        {
            ...
            if (lcmr.getLaneChange() != null)
            {
                ...
                for (Lane newLane : newLaneSet)
                {
                    ...
                    {
                        if (laneBasedGTU.getPerception().getAccessibleAdjacentLanes().get(oldLane)
                            .get(lcmr.getLaneChange()).contains(newLane))
                        {
        ...
        throws NetworkException, ValueException, GTUException
    {
        ...
        Length.Rel rightSuitability = laneDrop(gtu, LateralDirectionality.RIGHT);
        // @formatter:off
        if ((leftSuitability == NOLANECHANGENEEDED || leftSuitability == GETOFFTHISLANENOW)
            && currentSuitability == NOLANECHANGENEEDED
            && (rightSuitability == NOLANECHANGENEEDED || rightSuitability == GETOFFTHISLANENOW))
        ...
        throws NetworkException, GTUException
    {
        ...
        {
            System.err.println("GTU " + gtu.getId() + " has a route problem in suitability: " + ne.getMessage());
            return NOLANECHANGENEEDED;
        }
    }
        ...
        final Time.Rel timeHorizon) throws NetworkException
    {
        ...
        while (null != nextNode)
        {
            ...
            { // Look beyond this nextNode
                Link nextLink = gtu.getStrategicalPlanner().nextLinkDirection(nextNode, lastLink).getLink();
        ...
        Link linkAfterBranch = gtu.getStrategicalPlanner().nextLinkDirection(nextSplitNode, lastLink).getLink();
