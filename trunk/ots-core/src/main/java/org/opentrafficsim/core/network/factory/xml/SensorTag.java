 ...
 * License</a>.
 * <p>
 * $LastChangedDate: 2015-07-24 02:58:59 +0200 (Fri, 24 Jul 2015) $, @version
 * $Revision: 1147 $, by $Author: averbraeck $, initial version Jul 23, 2015
 * <br>
 * 
...
class SensorTag {
	 ...
	 * 
	 * @param node
	 *            the SENSOR node to parse
	 * @param parser
	 *            the parser with the lists of information
	 * @param linkTag
	 *            the parent LINK tag
	 * @throws SAXException
	 *             when parsing of the tag fails
	 * @throws NetworkException
	 *             when parsing of the tag fails
	 */
	@SuppressWarnings("checkstyle:needbraces")
	static void parseSensor(final Node node, final XmlNetworkLaneParser parser,
			final LinkTag linkTag) throws SAXException, NetworkException {
		...
		if (attributes.getNamedItem("LANE") == null)
			throw new SAXException("SENSOR: missing attribute LANE"
					+ " for link " + linkTag.name);
		String laneName = attributes.getNamedItem("LANE").getNodeValue().trim();
		if (linkTag.roadTypeTag == null)
			throw new NetworkException("SENSOR: LANE " + laneName
					+ " no ROADTYPE for link " + linkTag.name);
		CrossSectionElementTag cseTag = linkTag.roadTypeTag.cseTags
				.get(laneName);
		if (cseTag == null)
			throw new NetworkException("SENSOR: LANE " + laneName
					+ " not found in elements of link " + linkTag.name
					+ " - roadtype " + linkTag.roadTypeTag.name);
		if (cseTag.elementType != ElementType.LANE)
			throw new NetworkException("SENSOR: LANE " + laneName
					+ " not a real GTU lane for link " + linkTag.name
					+ " - roadtype " + linkTag.roadTypeTag.name);
		if (linkTag.generatorTags.containsKey(laneName))
			throw new SAXException("SENSOR for LANE with NAME " + laneName
					+ " defined twice");

		Node position = attributes.getNamedItem("POSITION");
		sensorTag.position = LinkTag.parseBeginEndPosition(
				position == null ? "END" : position.getNodeValue().trim(),
				linkTag);
		...
		if (classNode == null)
			throw new SAXException(
					"SENSOR: missing attribute CLASS for sensor "
							+ sensorTag.name + " on lane " + laneName);
		...
		try {
			...
			if (!AbstractSensor.class.isAssignableFrom(clazz))
				throw new SAXException("SENSOR: CLASS NAME "
						+ sensorTag.className + " for sensor " + sensorTag.name
						+ " on lane " + laneName
						+ " does not extend the AbstractSensor class");

			try {
				ClassUtil.resolveConstructor(clazz, new Class[] { Lane.class,
						DoubleScalar.Rel.class, RelativePosition.TYPE.class,
						String.class });
			...
			throw new SAXException("SENSOR: CLASS NAME " + sensorTag.className
					+ " for sensor " + sensorTag.name + " on lane " + laneName
					+ " could not be loaded.");
		}
		...
		if (triggerNode == null)
			throw new SAXException(
					"SENSOR: missing attribute TRIGGER for sensor "
							+ sensorTag.name + " on lane " + laneName);
		sensorTag.triggerPosition = parseTriggerPosition(triggerNode
				.getNodeValue().trim(), sensorTag, laneName);
	...
	/**
	 * @param trigger
	 *            String of the trigger position such as FRONT
	 * @param sensorTag
	 *            the sensor tag for error messages
	 * @param laneName
	 *            the lane name for error messages
	 * @return a relative position type, such as RelatievPostition.FRONT
	 * @throws SAXException
	 *             when the trigger position did not exist
	 */
	static RelativePosition.TYPE parseTriggerPosition(final String trigger,
			final SensorTag sensorTag, final String laneName)
			throws SAXException {
		switch (trigger) {
		case "FRONT":
		...
		default:
			throw new SAXException("SENSOR: wrong type of TRIGGER for sensor "
					+ sensorTag.name + " on lane " + laneName);
		}
	}
}
