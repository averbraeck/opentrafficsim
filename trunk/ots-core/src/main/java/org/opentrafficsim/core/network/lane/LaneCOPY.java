 ...
 */
public class LaneCOPY extends CrossSectionElement
{
    ...
    private Set<LaneCOPY> leftNeighbors = new HashSet<LaneCOPY>(1);

    /** Adjacent right lanes that some GTU types can change onto. */
    private Set<LaneCOPY> rightNeighbors = new HashSet<LaneCOPY>(1);

    /**
     * Next lane(s) following this lane. Initially null so we can calculate and cache the first time the method is
     * called.
     */
    private Set<LaneCOPY> nextLanes = null;

    /**
     * Next lane(s) following this lane. Initially null so we can calculate and cache the first time the method is
     * called.
     */
    private Set<LaneCOPY> prevLanes = null;
    ...
    public LaneCOPY(final CrossSectionLink<?, ?> parentLink, final DoubleScalar.Rel<LengthUnit> lateralOffsetAtStart,
     ...
     */
    private Set<LaneCOPY> neighbors(final LateralDirectionality direction)
    {
     ...
     */
    public final void addAccessibleAdjacentLane(final LaneCOPY adjacentLane, final LateralDirectionality direction)
    {
     ...
     */
    public final void removeAccessibleAdjacentLane(final LaneCOPY adjacentLane, final LateralDirectionality direction)
            throws NetworkException
    {
        Set<LaneCOPY> neighbors = neighbors(direction);
     ...
     */
    public final Set<LaneCOPY> nextLanes()
    {
        if (this.nextLanes == null)
        {
            // Construct (and cache) the result.
            this.nextLanes = new HashSet<LaneCOPY>(1);
            for (Link<?, ?> link : getParentLink().getEndNode().getLinksOut())
            {
                if (link instanceof CrossSectionLink<?, ?>)
                {
                    ...
                    {
                        if (cse instanceof LaneCOPY && laterallyCloseEnough(this, cse, LATERAL_MARGIN))
                        {
                            this.nextLanes.add((LaneCOPY) cse);
                        }
                    }
                }
            }
     ...
     */
    public final Set<LaneCOPY> prevLanes()
    {
        if (this.prevLanes == null)
        {
            // Construct (and cache) the result.
            this.prevLanes = new HashSet<LaneCOPY>(1);
            for (Link<?, ?> link : getParentLink().getStartNode().getLinksIn())
            {
                if (link instanceof CrossSectionLink<?, ?>)
                {
                    for (CrossSectionElement cse : ((CrossSectionLink<?, ?>) link).getCrossSectionElementList())
                    {
                        if (cse instanceof LaneCOPY && laterallyCloseEnough(cse, this, LATERAL_MARGIN))
                        {
                            this.prevLanes.add((LaneCOPY) cse);
                        }
                    }
                }
            }
     ...
     *         driving direction.
     */
    public final Set<LaneCOPY> accessibleAdjacentLanes(final LateralDirectionality lateralDirection,
            final GTUType<?> gtuType)
    {
        Set<LaneCOPY> candidates = new HashSet<>();
        for (LaneCOPY l : neighbors(lateralDirection))
        {
     ...
     *         does not match.
     */
    public final LaneCOPY bestAccessibleAdjacentLane(final LateralDirectionality lateralDirection,
            final DoubleScalar.Rel<LengthUnit> longitudinalPosition, final GTUType<?> gtuType)
    {
        Set<LaneCOPY> candidates = accessibleAdjacentLanes(lateralDirection, gtuType);
        ...
        LaneCOPY bestLane = null;
        double widthM = -1.0;
        for (LaneCOPY lane : candidates)
        {
