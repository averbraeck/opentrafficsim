...
public final class Test
{
            ...
            if (null == offsetLine)
            {
                if (printDetails)
                {
                    System.out.println(String.format(Locale.US,
                        "#offset %7.3f, method %3.3s returned null referenceLine %s", offset, offsetMethod,
                        referenceLine));
                }
                return false;
            }
            ...
            if (printDetails)
            {
                System.out.println(String.format(Locale.US,
                    "#offset %7.3f, method %3.3s: result line too close for %5.1f%%, "
                        + "too far for %5.1f%%, reference too close for %5.1f%%", offset, offsetMethod, resultTooClose
                        * factor, resultTooFar * factor, referenceTooClose * factor));
    ...
    public static void test5() throws OTSGeometryException
    {
        ...
        {
            double distance =
                centerLine.get(i).distanceToLineSegment(centerLine.get(0), centerLine.get(centerLine.size() - 1));
