...
public interface LaneBasedGTU<ID> extends GTU<ID>
{
     ...
     * returned. This method looks in all lanes where the GTU is registered, and not further than the given maxDistance.
     * When no vehicle is found within the given maxDistance, a very large value is returned. The search will extend
     * into successive lanes if the maxDistance is larger than the length of the lane. When Lanes (or underlying
     * CrossSectionLinks) diverge, the headway algorithms have to look at multiple Lanes and return the minimum headway
     * in each of the Lanes. When the Lanes (or underlying CrossSectionLinks) converge, "parallel" traffic is not taken
     * into account in the headway calculation. Instead, gap acceptance algorithms or their equivalent should guide the
     * merging behavior.<br>
    ...
    /**
     * Determine which GTU in front of us. This method looks in all lanes where the GTU is registered, and not further
     * than the given maxDistance. The minimum headway is returned of all Lanes where the GTU is registered. When no GTU
     * is found within the given maxDistance, <b>null</b> is returned. The search will extend into successive lanes if
     * the maxDistance is larger than the length of the lane. When Lanes (or underlying CrossSectionLinks) diverge, the
     * headway algorithms have to look at multiple Lanes and return the minimum headway in each of the Lanes. When the
     * Lanes (or underlying CrossSectionLinks) converge, "parallel" traffic is not taken into account in the headway
