...
public interface LaneBasedGTU<ID> extends GTU<ID>
{
     ...
     * to 0.0 meters. The relative position that will be registered is the referencePosition (dx, dy, dz) = (0, 0, 0). Front and
     ...
     * is found within the given maxDistance, a very large value is returned. The search will extend into successive lanes if
     * the maxDistance is larger than the length of the lane. When Lanes (or underlying CrossSectionLinks) diverge, the headway
     ...
     */
    DoubleScalar.Rel<LengthUnit> headway(DoubleScalar.Rel<LengthUnit> maxDistance) throws RemoteException, NetworkException;

    /**
     * Determine which GTU in front of us. This method looks in all lanes where the GTU is registered, and not further than the
     * given maxDistance. The minimum headway is returned of all Lanes where the GTU is registered. When no GTU is found within
     * the given maxDistance, <b>null</b> is returned. The search will extend into successive lanes if the maxDistance is larger
     * than the length of the lane. When Lanes (or underlying CrossSectionLinks) diverge, the headway algorithms have to look at
     * multiple Lanes and return the minimum headway in each of the Lanes. When the Lanes (or underlying CrossSectionLinks)
     * converge, "parallel" traffic is not taken into account in the headway calculation. Instead, gap acceptance algorithms or
     * their equivalent should guide the merging behavior.<br>
     * <b>Note:</b> Headway is the net headway and calculated on a front-to-back basis.
     * @param maxDistance the maximum distance to look for
     * @return DoubleScalarRel&lt;LengthUnit&gt;; the headway
     * @throws RemoteException when simulator time cannot be retrieved.
     ...
     * given maxDistance, <code>new DoubleScalar.Rel<LengthUnit>(Double.MAX_VALUE, LengthUnit.METER)</code> is returned. The
     ...
     *             lanes.
     */
    LaneBasedGTU<?> headwayGTU(Lane lane, DoubleScalar.Rel<LengthUnit> maxDistance) throws RemoteException, NetworkException;
    ...
    /**
     * Determine the time since last evaltuation when this GTU has covered the specified distance from the position of the last
     * evaluation time.
     * @param distance DoubleScalar.Rel&lt;LengthUnit&gt;; the distance
