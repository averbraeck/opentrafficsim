...
public class LaneTest
{
    ...
    public void laneConstructorTest() throws RemoteException, SimRuntimeException, NamingException, NetworkException
    {
        ...
        CrossSectionLink link =
            new CrossSectionLink("A to B", nodeFrom, nodeTo, new DoubleScalar.Rel<LengthUnit>(lineString.getLength(),
                LengthUnit.METER));
        ...
        Lane lane =
            new Lane(link, startLateralPos, endLateralPos, startWidth, endWidth, laneType, longitudinalDirectionality, f2000);
        ...
        link =
            new CrossSectionLink("A to B with Kink", nodeFrom, nodeTo, new DoubleScalar.Rel<LengthUnit>(lineString
                .getLength(), LengthUnit.METER));
        new LinearGeometry(link, lineString, null);
        lane =
            new Lane(link, startLateralPos, endLateralPos, startWidth, endWidth, laneType, longitudinalDirectionality, f2000);
        // Verify the easy bits
        ...
        approximateLengthOfContour =
            2 * (coordinates[0].distance(coordinates[1]) + coordinates[1].distance(coordinates[2])) + startWidth.getSI()
                + endWidth.getSI();
        assertEquals("Length of contour is approximately " + approximateLengthOfContour, approximateLengthOfContour, lane
            .getContour().getLength(), 4); // This lane takes a path that is about 3m longer
        assertEquals("Directionality should be " + longitudinalDirectionality, longitudinalDirectionality, lane
            .getDirectionality());
        assertEquals("There should be no GTUs on the lane", 0, lane.getGtuList().size());
        assertEquals("LaneType should be " + laneType, laneType, lane.getLaneType());
        ...
        assertEquals("Length of contour is approximately " + approximateLengthOfContour, approximateLengthOfContour, lane2
            .getContour().getLength(), 12); // This lane takes a path that is about 11 meters shorter
        ...
        for (int index = 1; index < numberOfCoordinates - 1; index++)
        {
            ...
            coordinates[index] =
                new Coordinate(nodeFrom.getPoint().x - radius + radius * Math.cos(angle), nodeFrom.getPoint().y + radius
                    * Math.sin(angle));
        ...
        link =
            new CrossSectionLink("Ring", nodeFrom, nodeTo, new DoubleScalar.Rel<LengthUnit>(lineString.getLength(),
                LengthUnit.METER));
        lineString = factory.createLineString(coordinates);
        new LinearGeometry(link, lineString, null);
        lane =
            new Lane(link, startLateralPos, startLateralPos, startWidth, startWidth, laneType, longitudinalDirectionality,
                f2000);
        ...
        new LinearGeometry(link, lineString, null);
        lane =
            new Lane(link, startLateralPos, startLateralPos, endWidth, endWidth, laneType, longitudinalDirectionality, f2000);
        // CrossSectionElement.printCoordinates("Lane contour", lane.getContour());
