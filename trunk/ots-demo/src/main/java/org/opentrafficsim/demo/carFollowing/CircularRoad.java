...
public class CircularRoad extends AbstractWrappableAnimation implements UNITS
{
    ...
    public CircularRoad() throws PropertyException
    {
        ...
        for (int lane = 1; lane <= 2; lane++)
        {
            ...
            outputProperties.add(new BooleanProperty(laneId + "Fixed Sample Rate Trajectories",
                laneId + " FSR Trajectories", laneId + "Trajectory (time/distance) diagram", true, false, 4));
            outputProperties.add(new BooleanProperty(laneId + "Variable Sample Rate Trajectories", laneId
                + " VSR Trajectories", laneId + "Trajectory (time/distance) diagram", true, false, 5));
        }
...
class RoadSimulationModel implements OTSModelInterface, UNITS
{
     ...
     */
    RoadSimulationModel(final ArrayList<AbstractProperty<?>> properties, final GTUColorer gtuColorer)
    {
        ...
        throws NamingException, NetworkException, SimRuntimeException, GTUException, OTSGeometryException
    {
        ...
        LaneBasedIndividualGTU gtu =
            new LaneBasedIndividualGTU("" + (++this.carsCreated), gtuType, vehicleLength, new Length(1.8, METER), new Speed(
                200, KM_PER_HOUR), this.simulator, this.network);
