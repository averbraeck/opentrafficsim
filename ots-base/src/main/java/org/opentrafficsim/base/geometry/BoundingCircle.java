        ...
        if (this.polygon == null)
        {
            this.polygon = new Polygon2d(new Iterator<Point2d>()
            {
                ...
                public Point2d next()
                {
                    ...
                    // at full circle (this.step == POLYGON_STEPS), make sure end point is exactly the same as the start point
                    double ang = this.step == POLYGON_STEPS ? 0.0 : (2.0 * Math.PI * this.step) / POLYGON_STEPS;
                    this.step++;
                    return new Point2d(Math.sin(ang) * BoundingCircle.this.radius, Math.sin(ang) * BoundingCircle.this.radius);
                }
            });
        }
        return this.polygon;
