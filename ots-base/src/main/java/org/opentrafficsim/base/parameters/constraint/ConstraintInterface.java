...
/**
 * In order to define default constraints within a Parameter Type, an <code>enum</code> is available. This interface supplies easy
 * access to the values of this <code>enum</code>. To use this interface, simply implement it as below. The value <code>POSITIVE</code>
 * is a property of this interface pointing to the <code>enum</code> field <code>POSITIVE</code>. As a result, the value that is set for
 * <code>X</code> is checked to be above zero. Note that model and parameterType do not have to be defined in the same class.
 * 
 * <pre>
 ...
 * 
 * <br>
 * In order to implement <i>custom</i> checks, any Parameter Type must extend the <code>check</code> method of its super. An example
 * is given below. The method should throw a <code>ParameterException</code> whenever a constraint is not met. The static
 * <code>throwIf</code> method is used to do this. The first check is a simple check on the SI value being above 2. The second check
 * compares the value with the value of another parameter in the <code>Parameters</code>. These checks can only be performed if the
 * other parameter is present in the <code>Parameters</code>. <b>Checks with other parameter type values should always check whether
 * <code>Parameters</code> contains the other parameter type</b>. i.e. <code>params.contains()</code>.<br>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * Checks are invoked on default values (if given), in which case an empty <code>Parameters</code> is forwarded. At construction of
 * a Parameter Type, no <code>Parameters</code> is available. Checks are also invoked when value are set into <code>Parameters</code>,
 * in which case that <code>Parameters</code> forwards itself. Even still, if in the above case X is set before Y, the check is
