...
public class ParameterTypes implements ConstraintInterface
{
    ...
    static
    {
        ...
        A = new ParameterTypeAcceleration("a", "Maximum (desired) car-following acceleration", Acceleration.instantiateSI(
            1.25), POSITIVE);

        B = new ParameterTypeAcceleration("b", "Maximum comfortable car-following deceleration", Acceleration.instantiateSI(
            2.09), POSITIVE)
        {
            /** */
            ...
            Acceleration.instantiateSI(3.5), POSITIVE)
        {
            ...
            {
                Acceleration b0 = params.getParameterOrNull(B0);
                Throw.when(b0 != null && value.si <= b0.si, ParameterException.class,
        ...
        B0 = new ParameterTypeAcceleration("b0", "Maximum adjustment deceleration, e.g. when speed limit drops", Acceleration
            .instantiateSI(0.5), POSITIVE)
        {
            /** */
            private static final long serialVersionUID = 20170203L;
        ...
        LOOKBACKOLD = new ParameterTypeLength("Look-back old", "Look-back distance (old version for MOBIL code)", Length
            .instantiateSI(-200.0), NEGATIVE);

        FSPEED = new ParameterTypeDouble("fSpeed", "Speed limit adherence factor", 1.0, POSITIVE);

        VCONG = new ParameterTypeSpeed("vCong", "Speed threshold below which traffic is considered congested", new Speed(60,
            SpeedUnit.KM_PER_HOUR), POSITIVE);

        LCDUR = new ParameterTypeDuration("lcDur", "Regular lane change duration", Duration.instantiateSI(3.0), POSITIVE);

        PERCEPTION = new ParameterTypeLength("perception", "Mental map length", new Length(2.0, LengthUnit.KILOMETER),
            POSITIVE);
