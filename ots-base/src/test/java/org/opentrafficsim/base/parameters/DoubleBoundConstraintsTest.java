...
import org.opentrafficsim.base.parameters.constraint.DoubleBound;
 ...
 */
public class DoubleBoundConstraintsTest
{

    /** Test values for the test. */
    private static double[] testValues = { Double.NEGATIVE_INFINITY, Double.MIN_VALUE, -100, -10, 0, 20, 200, Double.MAX_VALUE,
    ...
    public final void checkConstraint(final double low, final double high, final boolean includeLow, final boolean includeHigh)
    {
        if (high < low || high == low && (!includeLow) && (!includeHigh))
        {
            try
            {
                new DoubleBound(low, high, includeLow, includeHigh);
            }
        ...
        {
            DoubleBound db = new DoubleBound(low, high, includeLow, includeHigh);
            if (includeLow)
            {
            ...
            assertTrue("toString returns something sensible", db.toString().indexOf("DoubleBound") >= 0);
            for (double testValue : testValues)
            {
                boolean actualResult = db.fails(testValue);
                // System.out.println("testValue=" + testValue + ", DoubleBound=" + db.toString() + ", actualResult="
                // + actualResult);
                if (testValue < low)
                {
                    assertTrue("test value below range should fail", actualResult);
                }
                if (testValue == low && includeLow && low != high)
                {
                    assertFalse("test value at low end of range should not fail", actualResult);
                }
                ...
                {
                    assertFalse("test value within range should not fail", actualResult);
                }
                if (testValue == high && testValue != low && includeHigh)
                {
                    assertFalse("test value at high end of range should not fail", actualResult);
                }
                ...
                if (testValue > high)
                {
                    assertTrue("test value above range should fail", actualResult);
                }
            }
        }
    }
