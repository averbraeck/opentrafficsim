 ...
 */
public class TimeCircularRoadSimulation implements UNITS
{
    /** */
    ...
    public TimeCircularRoadSimulation()
    {
        try
        {
            ...
            if (!GRAPHS)
            {
                for (Property<?> ap : activeProperties)
                {
                    if (ap instanceof CompoundProperty)
                    {
                        ...
                        if ("OutputGraphs".equals(cp.getKey()))
                        {
                            while (cp.size() > 0)
                            {
                                cp.remove(0);
            ...
            SimpleAnimator sim = simulation.buildAnimator(Time.ZERO, Duration.ZERO, new Duration(3600.0, SECOND),
                    activeProperties, null, false);
            ...
            SwingUtilities.invokeLater(new Runnable()
            {
                ...
                public void run()
                {
                    simulation.getPanel().getTabbedPane().remove(0);
                    simulation.getPanel().getTabbedPane().remove(2);
                }
            });
    ...
    {
        System.out.println("Time of simulation was : " + (System.currentTimeMillis() - startTime) / 1000 + " seconds");
    ...
    public static void main(final String[] args)
    {
        ...
        new TimeCircularRoadSimulation();
    }
