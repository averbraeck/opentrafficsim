...
public final class ShapeFileReader
{
     ...
     * @throws IOException on error
     */
    public static void readLinks(final String shapeFileName, final Map<String, Link> links, final Map<String, Node> nodes,
        final OTSSimulatorInterface simulator) throws IOException
    {
        /*-
        ...
        try
        {
            ...
            for (int i = -1; i <= 1; i += 2)
            {
                ...
                for (int j = 0; j < n; j++)
                {
                    lat += i * 1.75;
                    Lane lane =
                        new Lane(link, new DoubleScalar.Rel<LengthUnit>(lat, LengthUnit.METER), m35, m35, null, dir, f200);
                ...
                for (int j = 0; j < spits; j++)
                {
                    lat += i * 1.75;
                    Lane lane =
                        new Lane(link, new DoubleScalar.Rel<LengthUnit>(lat, LengthUnit.METER), m35, m35, null, dir, f0);
                    new LaneAnimation(lane, simulator, Color.LIGHT_GRAY);
                    lat += i * 1.75;
                }
                Lane laneEO =
                    new Lane(link, new DoubleScalar.Rel<LengthUnit>(lat + i * 0.25, LengthUnit.METER), m05, m05, null,
                        LongitudinalDirectionality.NONE, f0);
        ...
        try
        {
            ...
            for (int i = -1; i <= 1; i += 2)
            {
                ...
                for (int j = 0; j < n; j++)
                {
                    lat += i * 1.5;
                    Lane lane =
                        new Lane(link, new DoubleScalar.Rel<LengthUnit>(lat, LengthUnit.METER), m30, m30, null, dir, f200);
