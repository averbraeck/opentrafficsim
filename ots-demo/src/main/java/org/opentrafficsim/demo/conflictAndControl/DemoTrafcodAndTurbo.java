...
/**
 * <p>
 * Copyright (c) 2013-2016 Delft University of Technology, PO Box 5, 2600 AA,
 * Delft, the Netherlands. All rights reserved. <br>
 * BSD-style license. See
 * <a href="http://opentrafficsim.org/node/13">OpenTrafficSim License</a>.
 * <p>
 * 
 * @version $Revision$, $LastChangedDate: 2017-01-30 18:25:56 +0100 (Mon,
 *          30 Jan 2017) $, by $Author$, initial version Dec 06,
 *          2016 <br>
	...
	class TrafCODModel extends EventProducer implements OTSModelInterface, EventListenerInterface {
		...
		public void constructModel(final SimulatorInterface<Time, Duration, OTSSimTimeDouble> theSimulator)
			...
			try {
				...
				Length headDetectorMargin = stopLineMargin.plus(headDetectorLength)
						.plus(new Length(3, LengthUnit.METER));
				Length longDetectorLength = new Length(30, LengthUnit.METER);
				Length longDetectorMargin = stopLineMargin.plus(longDetectorLength)
						.plus(new Length(10, LengthUnit.METER));
				int stream = 1;
				...
				DemoTrafcodAndTurbo.this.trafCOD = new TrafCOD(controllerName,
						URLResource.getResource("/conflictAndControl/Intersection12Dir.tfc"), trafficLights, sensors,
						(DEVSSimulator<Time, Duration, OTSSimTimeDouble>) theSimulator,
						DemoTrafcodAndTurbo.this.controllerDisplayPanel);
				DemoTrafcodAndTurbo.this.trafCOD.addListener(this,
				...
				DemoTrafcodAndTurbo.this.trafCOD.addListener(this,
						TrafficController.TRAFFICCONTROL_CONFLICT_GROUP_CHANGED);
				...
				DemoTrafcodAndTurbo.this.trafCOD.addListener(this,
						TrafficController.TRAFFICCONTROL_TRACED_VARIABLE_UPDATED);
		...
		public void notify(final EventInterface event) throws RemoteException {
			...
			if (TrafficController.TRAFFICCONTROL_CONTROLLER_EVALUATING.equals(type)) {
				...
				System.out.println(
						"Conflict group changed from " + ((String) payload[1]) + " to " + ((String) payload[2]));
		 ...
		 * 
		 * @param lane
		 *            Lane; the lane on which the block is placed
		 * @param theSimulator
		 *            the simulator
		 * @param gtuColorer
		 *            the gtu colorer to use
		 ...
		 *             on ???
		 * @throws GTUException
		 *             when construction of the GTU (the block is a GTU) fails
		...
		private Lane setupBlock(final Lane lane, OTSDEVSSimulatorInterface theSimulator, GTUColorer gtuColorer)
			...
			Duration tSafe = new Duration(0, TimeUnit.SECOND);
			...
			LaneBasedIndividualGTU block = new LaneBasedIndividualGTU("999999", gtuType,
					new Length(1, LengthUnit.METER), lane.getWidth(1), Speed.ZERO, theSimulator,
					(OTSNetwork) this.network);
			LaneBasedStrategicalPlanner strategicalPlanner = new LaneBasedStrategicalRoutePlanner(
					behavioralCharacteristics, new LaneBasedGTUFollowingTacticalPlanner(carFollowingModelCars, block),
					block);
