...
public class ScheduleCheckPulses<ID>
{
    ...
    public ScheduleCheckPulses(GTUType gtuType, OTSDEVSSimulatorInterface simulator, HashMap<String, CheckSensor> mapSensor,
        double backRange, double frontRange, List<CompleteRoute> routes, BufferedWriter outputFileLogVehicleSimulation)
        throws RemoteException, SimRuntimeException, NetworkException, GTUException, NamingException
    {
        ...
        this.lengthCar = 4.5;
        scheduleCheckPulses(backRange, frontRange, outputFileLogVehicleSimulation);
    }
     ...
     * @throws GTUException
     */
    public void scheduleCheckPulses(final double backRange, final double frontRange,
        BufferedWriter outputFileLogVehicleSimulation) throws RemoteException, SimRuntimeException, GTUException,
        ...
        for (Entry<String, CheckSensor> entry : this.mapSensor.entrySet())
        {
            ...
            for (Entry<DoubleScalar.Abs<TimeUnit>, Integer> entryPulse : pulses.entrySet())
            {
                if (entryPulse.getValue() == 0)
                {
                    ...
                    this.simulator.scheduleEventAbs(when, this, this, "findNearestVehicles", new Object[]{sensor, backRange,
                        frontRange, outputFileLogVehicleSimulation});
                }
            }

        }
        this.simulator.scheduleEventAbs(new DoubleScalar.Abs<TimeUnit>(0, TimeUnit.SECOND), this, this,
            "checkTriggeredVehiclesList", new Object[]{GTM.listGTUsInNetwork, backRange, outputFileLogVehicleSimulation});
    }

    private final void checkTriggeredVehiclesList(
        final HashMap<LaneBasedIndividualCar, LinkedList<CheckSensor>> listGTUsInNetwork, final double backRange,
        BufferedWriter outputFileLogVehicleSimulation) throws RemoteException, NetworkException, SimRuntimeException
    {
        ...
        for (Entry<LaneBasedIndividualCar, LinkedList<CheckSensor>> entry : listGTUsInNetwork.entrySet())
        {
            // destroy the gtu if it is not detected within a certain range
            if (!entry.getValue().isEmpty())
            {
                if (entry.getValue().get(0).getLocation().distance(entry.getKey().getLocation()) > backRange)
                {
        ...
        this.simulator.scheduleEventRel(new DoubleScalar.Rel<TimeUnit>(0.1, TimeUnit.SECOND), this, this,
            "checkTriggeredVehiclesList", new Object[]{GTM.listGTUsInNetwork, backRange, outputFileLogVehicleSimulation});
    }

    private final void findNearestVehicles(final CheckSensor sensor, final double backRange, final double frontRange,
        BufferedWriter outputFileLogVehicleSimulation) throws RemoteException, NetworkException
    {
        // is the sensor on an exit near a traffic light?
            ...
            if (gtu != null)
            {
                ...
                if (Settings.getBoolean(simulator, "ANIMATERAMPVEHICLES"))
                    ...
                    // the car is not triggered again!)
                        new DoubleScalar.Rel<LengthUnit>(sensor.getLongitudinalPositionSI() + 1, LengthUnit.METER), Integer
                            .parseInt(gtu.getId().toString()));

                try
                {
                    outputFileLogVehicleSimulation.write("t=" + simulator.getSimulatorTime().get().getSI() + " - gtu " + gtu
            ...
            else
            {
                ...
                try
                {
                    generateCar(
                        ...
                        // put the car one meter in front of the sensor (so
                        // the car is not triggered again!)
                        new DoubleScalar.Rel<LengthUnit>(sensor.getLongitudinalPositionSI() + 1, LengthUnit.METER),
                        (++ScheduleGenerateCars.carsCreated));
                    outputFileLogVehicleSimulation.write("t=" + simulator.getSimulatorTime().get().getSI()
                        + " - no near GTU found for exit lane " + sensor.getLane() + "\n");
    ...
    private final LaneBasedGTU nearGTUfront2(CheckSensor sensor, double range) throws RemoteException
    {
        ...
        return nearGTU2(sensor, sensor.getLane().getParentLink(), range, backwards);
    }
    ...
    private final LaneBasedGTU nearGTU2(CheckSensor sensor, CrossSectionLink link, double range, boolean backwards)
        throws RemoteException
    {
            ...
            if (cse instanceof Lane)
            {
                ...
                for (Object g : lane.getGtuList())
                {
                    ...
                    if (gtu instanceof LaneBasedIndividualCar)
                    {
                        ...
                        {
                            distanceSI = d;
                            if (GTM.listGTUsInNetwork.get((LaneBasedIndividualCar) gtu) != null && !backwards)
                            {
                                nearestGTU = gtu;
                                GTM.listGTUsInNetwork.remove(gtu);
                            }
                ...
                if (backwards)
                {
                    for (Object o : link.getStartNode().getLinksIn())
                    {
                        if (nearestGTU == null)
                        {
                            ...
                            nearestGTU = nearGTU2(sensor, prevLink, range, backwards);
                        }
                    }
                }
                else
                    ...
                    if (link.getEndNode().getLinksOut().size() > 0)
                    {
                        for (Object o : link.getEndNode().getLinksOut())
                        {
                            if (nearestGTU == null)
                            {
                                ...
                                nearestGTU = nearGTU2(sensor, nextLink, range, !backwards);
                            }
                        }
                    }
                }
        ...
        throws NetworkException
    {
        ...
        if (!ScheduleGenerateCars.enoughSpace(lane, initialPosition.getSI(), this.lengthCar, genSpeedSI, this.gtuType))
        {
            try
            {
                this.simulator.scheduleEventRel(new DoubleScalar.Rel<TimeUnit>(0.25, TimeUnit.SECOND), this, this,
                    "generateCar", new Object[]{lane, initialPosition, gtuNumber});
                return;
            }
        ...
        {
            CrossSectionLink link = (CrossSectionLink) node.getLinksOut().iterator().next();
            node = link.getEndNode();
            route.addNode(node);
        }
        LaneBasedRouteNavigator routeNavigator = new CompleteLaneBasedRouteNavigator(route);
        try
        {
