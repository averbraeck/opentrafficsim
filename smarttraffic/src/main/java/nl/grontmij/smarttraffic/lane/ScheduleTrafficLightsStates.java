...
public class ScheduleTrafficLightsStates<ID>
{
    ...
    TrafficLight trafficLightOnOff = null;

    public ScheduleTrafficLightsStates(OTSDEVSSimulatorInterface simulator, HashMap<String, CheckSensor> mapSensor,
        HashMap<String, StopLineLane> mapSignalGroupToStopLineAtJunction) throws RemoteException, SimRuntimeException,
     ...
     * @throws GTUException
     */
    public void scheduleTrafficLightsStatesFromDetector() throws RemoteException, SimRuntimeException, GTUException,
        NetworkException, NamingException
    {
        for (Entry<String, StopLineLane> entry : this.mapSignalGroupToStopLineAtJunction.entrySet())
        {
            entry.getKey();
            StopLineLane stopLine = entry.getValue();
            HashMap<DoubleScalar.Abs<TimeUnit>, Long> pulses = stopLine.getMapStopTrafficState();
            for (Entry<DoubleScalar.Abs<TimeUnit>, Long> entryPulse : pulses.entrySet())
            {
