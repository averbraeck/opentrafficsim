...
public class GTM extends AbstractWrappableSimulationST
{
    ...
    class GTMModel implements OTSModelInterface
    {
            ...
            throws SimRuntimeException, RemoteException
        {
            ...
            String networkFileName = Settings.getString(this.simulator, "NETWORK");
            ...
            // index and name
            String dirConfigVri = Settings.getString(this.simulator, "CONFIGDIR");// vri configuratie
            String dirLoggings = Settings.getString(this.simulator, "LOGGINGDIR");// VRI-loggings;
            // output;
            ...
            String[] vriNummer = Settings.getStringArray(this.simulator, "VRINUMMERS");//
            ...
            String dirExperiment = outputDir + "/" + Settings.getString(this.simulator, "EXPERIMENTDIR");

            outputFileVehiclesTriggered =
                Output.initiateOutputFile(dirExperiment, this.simulator, "reportVehiclesPassingCheckSensors.xls",
                    "Time\tNrCars\n");

            outputFileLogConfigVRI =
                Output.initiateOutputFile(dirExperiment, this.simulator, "ConfigVri.log",
                    "Read config files of traffic light regulated junctions: \n");

            BufferedWriter outputFileReportNumbers =
                Output.initiateOutputFile(dirExperiment, this.simulator, "reportNumbers.xls", "Time\tNrCars\n");

            // Lees hier de file met het netwerk
            URL url = URLResource.getResource(dirBase + "/input/model/" + networkFileName);
            ...
            configVriList =
                ConfigFile.readVlogConfigFiles(dirBase + "/" + "input" + "/" + dirConfigVri + "/", wegNummer, vriNummer,
                    outputFileLogConfigVRI);
            ...
            outputFileLogReadSensor =
                Output.initiateOutputFile(dirExperiment, this.simulator, "ReadAndCheckSensor.log",
                    "Read config files of traffic light regulated junctions: \n");
            ReadVLog.readVlogZipFiles(mapSensor, configVriList, timeVLog, dirBase + "/" + "input" + "/" + dirLoggings + "/",
                vLogFileName, wegNummer, vriNummer, this.simulator, startAtHour, stopAtHour, outputFileLogReadSensor);
            ...
            new ScheduleGenerateCars(GTUTYPE, this.simulator, mapSensorGenerateCars, generateCar, routes);

            new ReportNumbers(network, this.simulator, outputFileReportNumbers);

            outputFileMeasures =
                Output.initiateOutputFile(dirExperiment, this.simulator, "measure.xls", "Time\tSensor\tCar\n");
            Path file = Paths.get(GTM.CURRENTPROPERTIESFILENAME).getFileName();
            try
            {
                Files.copy(Paths.get(dirBase + "/" + "/input/" + file), Paths.get(dirExperiment + "/" + file),
            ...
            if (Settings.getBoolean(this.simulator, "MOVERAMPS"))
            {
                ...
                outputFileLogVehicleSimulation =
                    Output.initiateOutputFile(dirExperiment, this.simulator, "vehicleDetectections.log", "Time\tRemark\n");
                try
                {
