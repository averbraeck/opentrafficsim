 ...
 * <p>
 * This class acts as a transparent wrapper around the standard OTS {@code CarFollowingUtil}.
 * It automatically injects the Keane and Gao (2021) 2-parameter relaxation buffers
 * for specific leader GTUs. Additionally, it simplifies the method signatures for MiRoVA
 * by extracting repetitive parameters (model, speed, limits) directly from the tactical planner.
 * </p>
 * <p>
...
public final class MirovaCarFollowingUtil
{
    ...
    /* =========================================================================================
     * 1) RELAXED METHODS (Applicable for physical GTUs with IDs)
     * ========================================================================================= */

    /**
     * Calculates the acceleration towards a single leader, transparently applying ID-based relaxation.
     * <p>
     * This method utilizes a single-tick cache stored within the {@code EgoContext} to prevent
     * redundant car-following evaluations of the same leader GTU within the same simulation step.
     * </p>
     *
            ...
            throws ParameterException, GtuException
    {
        ...
        Acceleration result = CarFollowingUtil.followSingleLeader(
    ...
    /**
     * Calculates the most restrictive acceleration towards a set of multiple leaders,
     * evaluating ID-based relaxation for each leader individually.
     *
     * @param vehicle the tactical planner of the ego vehicle containing all contexts
    ...
    /* =========================================================================================
     * 2) STANDARD METHODS (No Relaxation - Target is virtual or lacks an ID)
     * ========================================================================================= */

    /**
     * Follows an arbitrary distance and speed without a specific GTU ID.
     * <p>
     * Note: No relaxation is applied here, as relaxation requires tracking a specific
     * vehicle ID over time.
     * </p>
     *
     ...
     * @throws GtuException       if GTU state cannot be accessed
     */
    public static Acceleration followDistanceAndSpeed(final MirovaTacticalPlanner vehicle, final Length distance, final Speed leaderSpeed)
            throws ParameterException, GtuException
    {
        return CarFollowingUtil.followSingleLeader(
                vehicle.getCarFollowingModel(),
                vehicle.getParameters(),
     ...
     * <p>
     * Note: Stopping is usually done for static infrastructure (e.g., traffic lights).
     * Therefore, the Keane and Gao relaxation phenomenon is physically not applicable here.
     * </p>
     *
     * @param vehicle  the tactical planner of the ego vehicle
    ...
    public static Acceleration constantAccelerationStop(final MirovaTacticalPlanner vehicle, final Length distance)
            throws ParameterException, GtuException
    {
        return CarFollowingUtil.constantAccelerationStop(
                vehicle.getCarFollowingModel(),
                vehicle.getParameters(),
                vehicle.getContext(EgoContext.class).getEgoSpeed(),
                distance
     ...
     * @throws GtuException       if GTU state cannot be accessed
     */
    public static Acceleration freeAcceleration(final MirovaTacticalPlanner vehicle)
            throws ParameterException, GtuException
    {
        return CarFollowingUtil.freeAcceleration(
                vehicle.getCarFollowingModel(),
                vehicle.getParameters(),
                vehicle.getContext(EgoContext.class).getEgoSpeed(),
                vehicle.getContext(InfrastructureContext.class).getCurrentSpeedLimit()
    ...
    /**
     * Returns an acceleration based on the car-following model in order to adjust the speed
     * to a given value at some location ahead (e.g., for early anticipation).
     * <p>
     * Note: This method operates on a virtual stationary vehicle internally. Relaxation
     * cannot and should not be applied here.
     * </p>
     *
     * @param vehicle     the tactical planner of the ego vehicle
     ...
     * @throws GtuException       if GTU state cannot be accessed
     */
    public static Acceleration approachTargetSpeed(final MirovaTacticalPlanner vehicle, final Length distance, final Speed targetSpeed)
            throws ParameterException, GtuException
    {
        return CarFollowingUtil.approachTargetSpeed(
                vehicle.getCarFollowingModel(),
                vehicle.getParameters(),
                vehicle.getContext(EgoContext.class).getEgoSpeed(),
                vehicle.getContext(InfrastructureContext.class).getCurrentSpeedLimit(),
