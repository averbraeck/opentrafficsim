 ...
 * desire, or only for a single lane change incentive. Desire is defined as ranging from -1 to 1, where 0 means no desire and 1
 * means full desire. Values above 1 are not valid and are limited to 1. Values below 0 are allowed and reflect that a
...
public record Desire(double left, double right)
{
    ...
    /**
     * Constructor which sets the supplied desire. Desire is bounded to [-1 1].
     * @param left Left desire.
     * @param right Right desire.
     */
    public Desire(final double left, final double right)
    {
        this.left = left <= 1.0 ? (left >= -1.0 ? left : -1.0) : 1.0;
        this.right = right <= 1.0 ? (right >= -1.0 ? right : -1.0) : 1.0;
    }
