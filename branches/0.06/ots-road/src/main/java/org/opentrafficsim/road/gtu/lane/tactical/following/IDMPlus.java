...
public class IDMPlus extends AbstractGTUFollowingModel {
    ...
    public final Acceleration computeAcceleration(final Speed followerSpeed, final Speed followerMaximumSpeed,
        ...
        Acceleration logWeightedAccelerationTimes2 = new Acceleration(Math.sqrt(this.a.getSI() * this.b.getSI()),
            AccelerationUnit.SI).multiplyBy(2);
        // don't forget the times 2

        Speed dV = followerSpeed.minus(leaderSpeed);
        Length.Rel sStar = this.s0.plus(followerSpeed.multiplyBy(this.tSafe)).plus(dV.multiplyBy(followerSpeed.divideBy(
